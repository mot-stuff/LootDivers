...
test("an unreachable character list falls back to local play with a notice", async ({
  page,
}) => {
  const state = freshMockState();
  await installAccountMock(page, state);
  // The session resolves but the character list dies on the network.
  await page.route("**/api/characters", (route) => route.abort("failed"));
  await page.goto("/play/?accountTest", { waitUntil: "networkidle" });
  await menuReady(page);
...
test("a signed-out session keeps the local menu with a homepage login pointer", async ({
  page,
}) => {
  await page.route("**/api/auth/session", (route) =>
    route.fulfill(
  ...
  const login = page.getByTestId("main-menu-login");
...
test("plain local loads and ?autostart never touch the account API", async ({
  ...
  // A real local player boot: the pre-709 menu, no login pointer, no
  ...
  await expect(page.getByTestId("main-menu-login")).toHaveCount(0);
  await expect(page.getByTestId("account-select")).toHaveCount(0);
