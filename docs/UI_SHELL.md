...
`test:browser` uses Vitest Browser Mode with the pinned Playwright Chromium
provider for Preact component tests. `test:smoke` serves the exact `dist`
artifact and includes focus, resize, DPR, startup, console, and network checks.
