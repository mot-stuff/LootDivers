...
## Site architecture: one Vite multi-page build — "/" homepage, "/play/" game
...
Pages project. `index.html` is a light game-site homepage — Loot Divers
branding, a Play link, login/signup forms against the Phase 8 v1 API
contract, and a news section — and the game shell moved verbatim to
...
- **Homepage (`src/home/`):** static-first HTML plus one small plain
  ...
  code on `/` — the game ships only in the `play` entry chunk. The
...
- **Auth forms:** POST `{email, password}` to `/auth/signup` and
  `/auth/login` per the §2 kickoff contract with cookie credentials.
  Client-side validation (email shape, password ≥ 8). The API host is
...
- **Game move:** all boot logic, `?autostart`, and the other automation
  ...
  `/play/assets/…`).
- **Mechanical churn:** every e2e `page.goto("/…")` became
  `page.goto("/play/…")` (32 call sites across 14 specs, precedent:
...
cross-origin cookies). See the Phase 8 kickoff §1.1.
...
- Until TASK-707 ships, real submissions always land in the documented
  ...
  proven against Playwright route mocks of the §2 contract.
